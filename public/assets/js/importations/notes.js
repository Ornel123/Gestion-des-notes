                ...
                if(list.length > 1)
                {
                    for(let [index, row] of list.entries()){
                        if(index > 0 && row !== ""){
                            ...
                            let note = row.split(";")[3].replace("\r", "").replace("\t", "");
...
function addNoteToList(noteData, shouldRefreshList = true){
    notesList.push({
        id: generateUniqueId(),
        ...noteData
    });
...
function setNotesListTableContent(){
    notesList.sort((a, b) => a.matricule_etudiant.localeCompare(b.matricule_etudiant));
